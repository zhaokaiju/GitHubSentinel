...
def generate_cnblogs_topic(model_type, model_name):
    config.llm_model_type = model_type
    ...
    markdown_file_path = cnblogs_client.export_top_articles()
    report, report_file_path = report_generator.generate_cnblogs_topic_report(markdown_file_path)
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("GitHub 项目进展"):
        ...
        # 创建 Radio 组件
        model_type = gr.Radio(["deepseek", "ollama"], label="模型类型", info="使用 DeepSeek API 或 Ollama 私有化模型服务")
    ...
    # 创建 博客园精华区话题 Tab
    with gr.Tab("博客园精华区话题"):
        gr.Markdown("## 博客园精华区话题")  # 添加小标题
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_cnblogs_topic, inputs=[model_type, model_name, ], outputs=[markdown_output, file_output])
